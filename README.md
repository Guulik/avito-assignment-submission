...
Бывает, что база не успевает создаться и приложение падает. Перезапуск решает 
</details>
...
Вроде сервер неплохо себя показывает:)
Тут ещё стоит сделать ВАЖНОЕ замечание, что запускается это всё дело через докер, но на моей локальной машине, которая
довольно ограничена в ресурсах. Ноут у меня не очень мощный - CPU:Ryzen 5 4600 RAM: 16GB (из которых обычно свободно не
...
Собственно, они все лежат в папке [tests](tests/), запускаются при запуске сервера и проходят проверку.
